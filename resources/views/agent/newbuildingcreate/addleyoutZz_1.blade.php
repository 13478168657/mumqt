        ...
        @if(!empty($room->items()))
          ...
          <tr @if( ( $rkey + 1) % 2 == 0) class="backColor" @endif >
            ...
            <td>
              ...
              @else
              商铺&nbsp;&nbsp;{{$rval->floorage}}平米&nbsp;&nbsp;{{config('faceToConfig.'.$rval->faceTo)}}
              @endif
            </td>
            <td>{{$rval->timeCreate}}</td>
            <td value="{{$rval->id}}" ><a class="modaltrigger" value="{{json_encode($rval, JSON_UNESCAPED_UNICODE)}}" onclick="edithuxing(this);" href="#add">修改</a>&nbsp;&nbsp;<a href="#sc" class="modaltrigger" onclick="deletehuxing(this);">删除</a></td>
          </tr>
          @endforeach
        @else
        <tr>
...
<div class="main_r add" id="add" style="top:150px;">
  ...
  <div class="write_msg" style="width:700px;">
    <ul class="input_msg">
        <li>
          ...
          <span class="res_roomType" style="color:red;margin-left:10px;"></span>
        </li>
        <li>
...
/** 户型信息-商铺 **/
  ...
  var roomType, roomName, weizhi, faceTo, floorage, usableArea, num, state, price, cbIds, uIds = '', feature, leyout, rId, delImg = new Array();
  ...
  $('#next').click(function(){
    ...
    if(roomType && roomName && weizhi && floorage && usableArea && num && state && price && leyout){
  ...
  $('.cbIds').click(function(){
    ...
    if($(this).prop('checked')){
        ...
        uIds2.children('li').each(function(){
          ...
          for( i in uIds2_all){
            // console.log($(this).children('input.uIds').val());
...
function edithuxing(obj){
    ...
    if( typeof image != 'null' || typeof image != 'undefined'){
        ...
        for( var i in image){
            ...
            lis += '<div class="cz"><input type="text" placeholder="描述/别名" class="diyFileName" value="' + image[i].note + '">';       
            lis += '<input type="hidden" class="photo_path" value="'+ '{{config("imgConfig.imgSavePath")}}' +image[i].fileName +'"></div>';       
            lis += '</li>';
        }
...
function getImage(obj){
  ...
  sonList.each(function(index){
      images[index] = {
          img:$(this).find('input.photo_path').val(),
