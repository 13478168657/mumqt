<li style="height:auto; overflow:hidden;">
  ...
  @if(!empty($info['biaoti'][0]->fileName))
    <div class="title_img">
      <img id="title_img" value="{{config('imgConfig.imgSavePath')}}{{$info['biaoti'][0]->fileName}}" src="{{config('imgConfig.imgSavePath')}}{{$info['biaoti'][0]->fileName}}">
      <input type="hidden" id="title_id" value="{{$info['biaoti'][0]->id}}"/>
    </div>
   @else
    <div class="title_img">
...
<script>
  ...
  function getImage(obj){
      ...
      if(sonList.length < type.num){
          xalert({
              title:'提示',
              content:type.tname + '至少要' + type.num + '张!',
          });
          return false;
      }
      sonList.each(function(index){
          images[index] = {
              img:$(this).find('.photo_path').val(),
  ...
  $('#saveImg').click(function(){
      ...
      var comid  = $('#communityId').val();

      var title = {'id':$('#title_id').val(), 'img':$('#title_img').attr('value')}; //标题图
