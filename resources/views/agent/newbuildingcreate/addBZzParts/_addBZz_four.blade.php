...
@if($typeInfo == 301 || $typeInfo == 302)
    ...
    <ul class="input_msg enter_build">
        ...
        <li>
            ...
            <input type="text" class="txt width2" name="houseTotal" datatype="*" nullmsg="请填写总户数" @if(!empty($type2GetInfo)) value="{{$type2GetInfo->houseTotal}}" @endif />
            <span class="tishi">户</span>
        </li>
        <li>
            <label><span class="dotted colorfe">*</span>容积率：</label>
            <input type="text" class="txt width2" name="volume" datatype="/^(([1-9]\d*$)|(^0\.\d{1,2}$)|(^[1-9]\d*\.\d{1,2}))$/" nullmsg="请填写容积率" errormsg="容积率为正整数或1-2位小数" @if(!empty($type2GetInfo)) value="{{$type2GetInfo->volume}}" @endif />
