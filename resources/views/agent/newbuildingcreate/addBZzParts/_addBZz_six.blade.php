    ...
    <ul class="input_msg enter_build">
        ...
        <li class="no_height">
            ...
            @foreach($macroplate as $mac)
                <input type="radio" name="macroplateId" value="{{$mac->id}}" class="radio" datatype="*" nullmsg="请选择所属板块" @if(!empty($type2GetInfo)) @if($type2GetInfo->macroplateId == $mac->id) checked="checked" @endif @endif />
