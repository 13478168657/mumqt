          ...
          <ul class="input_msg">
            <li>
              ...
              <div class="dw" style="margin-left:0; margin-right:15px;">
                <a class="term_title"><span id="zheType1_2" value="">折扣</span><i></i></a>
                <div class="list_tag">
                   <p class="top_icon"></p>
                   <ul class="zheType1_2" >
    ...
    <div class="add" id="pq" style="width:900px; top:100px !important;">
      <h2>维护排期信息</h2>
      <span class="close" onClick="$(this).parent().hide(); $('#lean_overlay').hide();"></span>
          <div class="write_msg">
...
$(function(){
  ...
  //租房营销方式
  function getZu(){
    zuGao = testing('input[name="zuGao1"]', /^(([1-9]\d*$)|(^0\.\d{1,2}$)|(^[1-9]\d*\.\d{1,2}))$/, '最高租赁价格为正整数或1-2位小数', '#res_zuGao1');
    zuDi = testing('input[name="zuDi1"]', /^(([1-9]\d*$)|(^0\.\d{1,2}$)|(^[1-9]\d*\.\d{1,2}))$/, '最低租赁价格为正整数或1-2位小数', '#res_zuDi1');
    zuPing = testing('input[name="zuPing1"]', /^(([1-9]\d*$)|(^0\.\d{1,2}$)|(^[1-9]\d*\.\d{1,2}))$/, '平均租赁价格为正整数或1-2位小数', '#res_zuPing1');
    zuDesc = testing('#zuDesc1', /^\S.*$/, '租赁价格描述不能为空', '#res_zuDesc1');
  }
...
$(function(){
  ...
  $('#updatePrice1').click(function(){
    ...
    shouPing = testing('input[name="shouPing1_2"]', /^(([1-9]\d*$)|(^0\.\d{1,2}$)|(^[1-9]\d*\.\d{1,2}))$/, '平均售价为正整数1-2位小数', '#res_shouPing1_2');
  ...
  $('#updateZhe1').click(function(){
    ...
    if(!zheType){
        ...
        if(zheType == 1){
            ...
            alert($('input[name="zhiZhe1_2"]').val());
            jian = true;
        }else if(zheType == 2){
  ...
  $('#updateDianyou1').click(function(){
    ...
    if( !/^\S.*$/.test($('input[name="fu1_2"]').val()) || !/^\S.*$/.test($('input[name="di1_2"]').val()) ){
      ...
      $('#res_dianYou1_2').text('');
      dianYou = $('input[name="fu1_2"]').val() + '_' + $('input[name="di1_2"]').val();
    }
    if(dianYou){
      $.ajax({
        type:'post',
