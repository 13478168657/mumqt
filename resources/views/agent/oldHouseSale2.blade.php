...
<div class="main">
  ...
  <div class="main_r" id="main_r">
    <p class="right_title border_bottom htype">
        @if($class == 'sale')
    ...
    <div class="write_msg">
      ...
      <form action="/submit{{$class}}" method="post" enctype="mutltipart/form-data" id="oldsale">
        <input type="hidden" name="_token" value="{{ csrf_token() }}" >
          ...
          <input type="hidden" name="internalNum"  value="{{$internalNum}}"/>
          <input type="hidden" name="housingInspectionNum"  value="{{$housingInspectionNum}}"/>
          <input type="hidden" name="address"  value="{{$address}}"/>
          <input type="hidden" name="houseNum"  value="{{$houseNum}}"/>
      ...
      <ul class="input_msg">
        ...
        @elseif(($type == 'office')||($type == 'shops'))
          ...
          @if($class == 'sale')
              ...
              <li>
                <label><span class="dotted colorfe">*</span>楼层：</label>
                <span class="tishi margin">第</span>
                <input class="txt width3" type="text" name="currentFloor" value="<?=(!empty($house->currentFloor))?$house->currentFloor:''?>">
                <span class="tishi">层</span>
                <span class="tishi margin_l">共</span>
                <input class="txt width3" type="text" name="totalFloor" value="<?=(!empty($house->totalFloor))?$house->totalFloor:''?>">
                <span class="tishi">层</span>
                @if($type == 'shops')
                <span class="tishi color8d">地下室请填负值</span>
                @endif
              </li>
          @endif
          <li>
        ...
        <li>
          ...
          <div class="sort_icon" style="margin-right:15px;">
            ...
            <input type="hidden" name="buildingStructure" value="">
            <div class="list_tag"  style="width:150px;">
               <p class="top_icon"></p>
               <ul>
                 <li>选择结构</li>
          ...
          <div class="sort_icon">
            ...
            <input type="hidden" name="buildingType"  value="">
            <div class="list_tag"  style="width:150px;">
               <p class="top_icon"></p>
               <ul>
                 <li>选择类别</li>
        ...
        <li>
          <label><span class="dotted colorfe">*</span>楼层：</label>
          <span class="tishi margin">第</span>
          <input type="text" class="txt width3" name="currentFloor" value="<?=(!empty($house->currentFloor))?$house->currentFloor:''?>"/>
          <span class="tishi">层</span>
          <span class="tishi margin_l">共</span>
        ...
        @if($type == 'villa')
          <li>
            ...
            @if(!empty($faces))
                <?php $i=1;?>
              @foreach($faces as $k=>$face)
                @if((!empty($house->faceTo) && $house->faceTo == $k)||($i==1))
                  <input type="radio" name="faceTo" class="radio" checked="checked" value="{{$k}}"/>
                  <span class="tishi">{{$face}}</span>
                @else
                  <input type="radio" name="faceTo" class="radio"  value="{{$k}}"/>
          ...
          <label><span class="dotted colorfe">*</span>装修程度：</label>
            @if(!empty($fitments))
              ...
              @foreach($fitments as $k=>$fitment)
                @if((!empty($house->fitment) && $house->fitment == $k)||($i==1))
...
//房源分类跳转
    $('.htype a').bind('click',function(){
        var htype = $(this).attr('attr');
        $('#oldsale').attr('action','/oldsale2/'+htype);
        $('#oldsale').submit();
    });
//模板数量
    $('.ms').html($('.mb').length);
...
//房源标签选择
  $('.color_blue ').bind('click',function(){
