...
<p class="route">
  ...
  <a href="#">首页</a>
  <span>&nbsp;>&nbsp;</span>
    <a href="{{$linkurl}}">{{$cityName}} @if($subtype=='build')写字楼小区@else @if(strpos($type,'sale'))写字楼出售@else写字楼出租@endif @endif</a>
    @if(!empty($cityareaid))
        <span>&nbsp;>&nbsp;</span>
  ...
  <div class="list_nav">
    <div class="nav_l">
      ...
      <!--  <a  class="<?=($subtype == 'business')?'click color_blue border_blue':''?>" href="/{{$type}}/business">核心圈查询</a> -->
       ...
       @endif
        @if(strpos($type,'sale'))
            <a href="/map/sale/office">地图查询</a>
        @else
            <a href="/map/rent/office">地图查询</a>
        @endif
    </div>
  </div>
  <div class="list_term">
            ...
            <!--  <div class="home_detail"><a href="/housedetail/s{{$sr}}{{$topHouse->_source->id}}.html" class="img_num"><i></i>@if(!empty($topHouse->_source->imageCount)){{$topHouse->_source->imageCount}}@else 0  @endif</a></div> -->
              ...
              <dd class="margin_l">
                  <p class="build_name margin_b">
                      <a href="/housedetail/s{{$sr}}{{$topHouse->_source->id}}.html" class="name" onclick="staClick('{{$topHouse->_source->id}}','t')" target="_blank">{{$topHouse->_source->title}}</a>
                      &nbsp;&nbsp;&nbsp;&nbsp;
                  </p>
                      ...
                      @if(!empty($house->_source->address))
                          ...
                          <span title="{{$house->_source->address}}" class="address">{{$house->_source->address}}</span>
                              @if(!empty($house->_source->communityId))
                                  <a href="/map/{{($type=='xzlsale')?'sale':'rent'}}/office?communityId={{$house->_source->communityId}}&longitude={{$house->_source->longitude}}&latitude={{$house->_source->latitude}}"><i class="map_icon"></i></a>
                              @endif
                      </span>
                      @endif
                     @else
                      <span class="color8d">
                          ...
                          @if(!empty($topHouse->_source->address))
                              {{$topHouse->_source->address}}
                                  @if(!empty($topHouse->_source->communityId))
                                      <a href="/map/rent/office?communityId={{$topHouse->_source->communityId}}&longitude={{$topHouse->_source->longitude}}&latitude={{$topHouse->_source->latitude}}" target="_blank"><i class="map_icon"></i></a>
                                  @endif
                          @endif
                      </span>
                     @endif
                  ...
                  <!-- 写字楼置顶 -->
                    @if(strpos($type,'sale'))
                      <span>{{$housetypes[$topHouse->_source->houseType2] or '其它'}}</span>
                      <span>&nbsp;|&nbsp;</span>
                      @if(!empty($topHouse->_source->area))
                          <span class="area_width">{{floor($topHouse->_source->area)}}平米</span>
                          <span>&nbsp;|&nbsp;</span>
                      @endif
                      <span>所属楼层：<span>{{$topHouse->_source->currentFloor}}</span>&nbsp;/&nbsp;<span>{{$topHouse->_source->totalFloor}}</span>层</span>
                    @else
                      @if($subtype == 'build')
                        @if(!empty($typeInfo))
                            @if(isset($typeInfo->officeLevel))
                            <span>{{(($typeInfo->officeLevel != 3)&&!empty($officeLevels[$typeInfo->officeLevel]))?$officeLevels[$typeInfo->officeLevel].'级':'其它'}}</span>
                            <span>&nbsp;|&nbsp;</span>
                            @endif
                            <span>{{(!empty($type2)&&!empty($housetypes[$type2]))?$housetypes[$type2]:''}}</span>
                        ...
                        @endif
                      @else
                          @if($topHouse->_source->officeLevel !=='')
                          <span>{{!empty($officeLevels[$topHouse->_source->officeLevel])?$officeLevels[$topHouse->_source->officeLevel].'级':'其它'}}</span>
                          <span>&nbsp;|&nbsp;</span>
                          @endif
                          ...
                          @if(!empty($topHouse->_source->area))
                              ...
                              <span>&nbsp;|&nbsp;</span>
                          @endif
                          <span>共{{$topHouse->_source->totalFloor}}层</span>
                      @endif
                    @endif
                  </p>
   ...
   @if(!empty($resultfee))
    ...
    @foreach($resultfee as $house)
      ...
      <dl>
          <dt>
              @if($subtype != 'build')
                  <a href="/housedetail/s{{$sr}}{{$house->_source->id}}.html"><img src="@if(!empty($house->_source->thumbPic)){{get_img_url($objectType,$house->_source->thumbPic,2)}}@else{{$defaultImage}}@endif"  alt="标题图"></a>
          ...
          @else
              <a href="/esfindex/{{$house->_source->id}}/{{!empty($type2)?$type2:'201'}}.html"><img src="@if(!empty($house->_source->titleImage)){{get_img_url('commPhoto',$house->_source->titleImage,2)}}@else{{$defaultImage}}@endif"  alt="标题图"></a>
              @endif
          </dt>
              ...
              <p class="build_name margin_b">
                  @if($subtype != 'build')
                      ...
                      <!--<span class="home_time"><?php
                              ...
                              ?>前更新</span>-->
                  @else
                      <a href="/esfindex/{{$house->_source->id}}/{{!empty($type2)?$type2:'101'}}.html" class="name build_width" target="_blank">{{!empty($house->_source->name)?$house->_source->name:''}}</a>
                      @if(!empty($type2))
                          <a  class="state subway">{{$housetypes[$type2]}}</a>
                      @endif
                  @endif
              </p>
              <p class="finish_data color8d margin_b">
                  ...
                  @if($subtype != 'build')
                      ...
                      @if(!empty($house->_source->address))
                          <span>
                              <span title="{{$house->_source->address}}" class="address">{{$house->_source->address}}</span>
                              @if(!empty($house->_source->communityId))
                                  <a href="/map/{{($type=='xzlsale')?'sale':'rent'}}/office?communityId={{$house->_source->communityId}}&longitude={{$house->_source->longitude}}&latitude={{$house->_source->latitude}}" target="_blank"><i class="map_icon"></i></a>
                              @endif
                          </span>
                      @endif
                  @else
                      <span class="color8d">
                          ...
                          @if(!empty($house->_source->address))
                              {{$house->_source->address}}
                                  @if(!empty($house->_source->communityId))
                                      <a href="/map/rent/office?communityId={{$house->_source->communityId}}&longitude={{$house->_source->longitude}}&latitude={{$house->_source->latitude}}" target="_blank"><i class="map_icon"></i></a>
                                  @endif
                          @endif
                      </span>
                  @endif
              ...
              <p class="home_num color8d margin_b">
                  @if($type == 'xzlsale')
                      ...
                      @if(!empty($house->_source->area))
                          ...
                          <span>&nbsp;|&nbsp;</span>
                      @endif
                      <span>所属楼层：<span>{{$house->_source->currentFloor}}</span>&nbsp;/&nbsp;<span>{{$house->_source->totalFloor}}</span>层</span>
                  @else
                      @if($subtype == 'build')
                          @if(!empty($typeInfo))
                              @if(isset($typeInfo->officeLevel))
                          ...
                          @endif
                      @else
                          @if($house->_source->officeLevel !=='')
                          <span>{{!empty($officeLevels[$house->_source->officeLevel])?$officeLevels[$house->_source->officeLevel].'级':'其它'}}</span>
                          <span>&nbsp;|&nbsp;</span>
                          @endif
                          <span>{{$housetypes[$house->_source->houseType2] or '其它'}}</span>
                          <span>&nbsp;|&nbsp;</span>
                          @if(!empty($house->_source->area))
                              ...
                              <span>&nbsp;|&nbsp;</span>
                          @endif
                          <span>共{{$house->_source->totalFloor}}层</span>
                      @endif
                  @endif
              </p>
              <p class="build_tag">
          ...
          <dd class="right {{($subtype != 'build')?'price_r':''}}">
              ...
              <p class="handle">
                  ...
                  @else
                      <a class="color66"><span class="focus" value="{{$house->_source->id}},3,{{$housetype1}},0"><i class="follow"></i><span>{{(!empty($interest)&&(in_array($house->_source->id,$interest))?'已关注':'关注')}}</span></span></a>
...
@if(!empty($houses))
          ...
          <dd class="right {{($subtype != 'build')?'price_r':''}}">
              ...
              @else
                  ...
                  @else
                      ...
                      <p class="build_price margin_top">
                          {{--<a class="house_num"></a><a class="color_blue" style=" font-size:16px;" href="/esfrent/area/ba{{$house->_source->id}}">{{$house->_source->rentCount}}</a>&nbsp;&nbsp;个房源在租--}}
                          <a class="house_num"></a><a class="color_blue" style=" font-size:16px;" href="/xzlrent/area/ba{{$house->_source->id}}">{{$house->_source->rentCount}}</a>&nbsp;&nbsp;个房源在租
                      </p>
                  @endif
              @endif
              <p class="handle">
   ...
   @endforeach
    @if(!empty($resBool))
        ...
        @else
            <p class="more_home"><a class="color_blue" href="{{$linkurl}}">更多@if($type == 'spsale')写字楼出售@else写字楼出租@endif&gt;&gt;</a></p>
        @endif
    @endif
@else
...
@endif
  ...
  <div class="list_r">
      @if($subtype == 'area')
          <div class="function">
              @if(strpos($type,'sale'))
                  <a href="/houseHelp/sale/office"><i class="sale"></i>个人出售</a>
                  <a href="/houseHelp/sale/office/entrust"><i class="sale_entrust"></i>委托出售</a>
              @else
                  <a href="/houseHelp/rent/office"><i class="sale"></i>个人出租</a>
                  <a href="/houseHelp/rent/office/entrust"><i class="sale_entrust"></i>委托出租</a>
              @endif
          </div>
      @endif
...
<!--      	<div class="list_adv">
      ...
      <div class="spread_new">
        <p class="price_title"><a href="/office/area">热销新盘>></a></p>
