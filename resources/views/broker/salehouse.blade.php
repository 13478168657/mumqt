    ...
    <div class="msg_list" id="msg_list">
        ...
        <div class="home_list">
          <div class="house_list">
            <dl class="build_msg">
              ...
              <span class="colorfe">{{$pagev->avgPrice}}</span>元/套
              @else
            ...
            <dl class="build_house">
              <dt>
                  ...
                  <div class="callbacks_container">
                    <ul class="rslides" id="slider4">
                      @if(!empty($pagev->list[$i]->imageList))
                          ...
                          <li>
                              <a href="/housedetail/ss{{$pagev->list[$i]->id}}.html" target="_blank" title="{{$pagev->list[$i]->title}}">
                                <img data-original="{{get_img_url('houseSale', $imagev, 1)}}" alt="房源图片">
                              </a>
                          </li>
                          @endforeach
                      @elseif(!empty($pagev->list[$i]->thumbPic))
                        <a href="/housedetail/ss{{$pagev->list[$i]->id}}.html" target="_blank" title="{{$pagev->list[$i]->title}}">
                            <img data-original="{{get_img_url('houseSale', $pagev->list[$i]->thumbPic, 1)}}" alt="房源图片">
                        </a>
                      @else
                        <a href="/housedetail/ss{{$pagev->list[$i]->id}}.html" target="_blank" title="{{$pagev->list[$i]->title}}">
                ...
                <p class="house_img"><i></i>
                    {{count($pagev->list[$i]->imageList)}}
                    <span>
                        @if($pagev->list[$i]->priceUnit == 1)
                        {{$pagev->list[$i]->price1}}
                        <span class="font_size">元/平米</span>
                        @elseif($pagev->list[$i]->priceUnit == 2)
                        {{$pagev->list[$i]->price2}}
                        <span class="font_size">万元/套</span>
                        @endif
                    </span>
                </p>
              </dt>
              <dd>
                <p class="house_name">
                  <a href="/housedetail/ss{{$pagev->list[$i]->id}}.html" target="_blank">{{$pagev->list[$i]->title}}</a>
                </p>
                <p class="house_type">
      ...
      @else
        <div class="no_state no_border">
          ...
          <div class="state_r">
              <p class="p1">很抱歉，该经纪人没有相关的二手房房源！</p>
              <!-- <div class="p2">
