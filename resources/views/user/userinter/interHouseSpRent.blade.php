...
@section('content')
  ...
  <div class="user_r">
    ...
    <p class="type_nav">
		<a href="/user/interHouse/shopsale">出售</a>
		<a class="click" href="/user/interHouse/shoprent">出租</a>
    </p>
    <div class="data_list">
	@if(!empty($houseData))
        ...
        @if($house->found)
         <div class="data_msg">
            <dl>
              ...
              <dd class="data_price">
                <p class="price margin_t">@if(!empty(floor($house->_source->price2)))<span>{{floor($house->_source->price2)}}</span>元/平米/天@else<span>面议</span>@endif</p>
         ...
         @foreach($houseData['info'] as $info)
            <div class="data_msg">
                <dl>
                    ...
                    <dd class="data_price">
                        <p class="price margin_t">@if(!empty(floor($info->price2)))<span>{{floor($info->price2)}}</span>元/平米/天@else<span>面议</span>@endif</p>
	...
	@else
		<div class="no_data">
			...
			<div class="no_info">
				<p class="p1"><span>亲</span>，目前没有关注出租商铺！您还可以去看看其它出租的商铺房源</p>
				<p>去<a href="/sprent/area">关注商铺</a></p>
			</div>
		</div>
	@endif
    </div>
