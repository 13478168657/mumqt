  ...
  <div class="house_info">
    @if(!is_numeric($houseId))
      ...
      <p class="submenu">
        ...
        <a href="/houseHelp/rent/office">写字楼</a>
        <span class="click">商铺</span>
        @else
          <a href="/houseHelp/rent/xq/entrust">住宅</a>
          <a href="/houseHelp/rent/office/entrust">写字楼</a>
          <span class="click">商铺</span>
        @endif
      </p>
    @else
    ...
    <div class="house_type">
      @if(empty($entrust))
      <form id="saveRentSp" action="/houseHelp/save/rent/shop">
      @else
      <form id="saveRentSp" action="/houseHelp/save/rent/shop/entrust">
      @endif
      ...
      <ul>
        ...
        <li class="no_clear">
          ...
          <div class="chose_build">
            <input type="text" class="txt" onkeyup="searchCommunity(this,'1')" data-type="void" placeholder="请输入楼盘名称" @if(isset($houseDetail->name)) value="{{$houseDetail->name}}" @else value="" @endif>
        ...
        <li>
          ...
          <input type="text" class="txt width2 margin_l" data-type="officeRent" name="price2" @if(isset($houseDetail->price2)) value="{{$houseDetail->price2}}" @else value="" @endif>
        ...
        <li>
          <label><span>*</span>是否转让：</label>
          <input type="radio" class="radio js-yes" @if(!isset($houseDetail))checked="checked" @endif @if(isset($houseDetail->isTransfer) && $houseDetail->isTransfer == 1) checked="checked" @endif name="isTransfer" value="1">
          <span class="room">是</span>
          <input type="radio" class="radio js-no" @if(isset($houseDetail->isTransfer) && $houseDetail->isTransfer == 0) checked="checked" @endif name="isTransfer" value="0">
          <span class="room">否</span>
          <div class="charge" style="display: block;margin-left:40px;">
              <input type="text" name="transferPrice" @if(isset($houseDetail->transferPrice) && $houseDetail->isTransfer == 1 && $houseDetail->transferPrice != '0.00') value="{{floor($houseDetail->transferPrice)}}" @else value="" @endif style="border-color: rgb(203, 203, 203); float:left"><strong style="float: left;line-height: 30px;">&nbsp;万元&nbsp;&nbsp;</strong>
            	<span style="float: left;line-height: 30px;margin:0 10px;">无转让费不填写</span>
           		<span class="tip" style="float: left;"><i></i><em></em></span>
          </div>
        </li>
        {{--<li class="no_clear">--}}
      ...
      <ul>
        ...
        <li>
          ...
          <input type="text" class="txt margin_l js-tel" name="linkmobile" maxlength="11" data-type="tel" name="linkmobile" @if(Auth::check()) value="{{Auth::user()->mobile}}" @else data-check="mobile" value="" @endif>
        </li>
...
$(document).ready(function(e) {
  ...
  $('.charge input').on('keydown',function(e){
	...
	$('.charge input').on('blur',function(){
		var val=$('.charge input').val();
		if(val){
			if(!/^\d{1,5}$/.test(val)){
				$('.charge .tip i').attr('class','error');
				$('.charge .tip em').html('请输入不超过5位数的数字');
			}else{
				$('.charge .tip i').attr('class','ok');
				$('.charge .tip em').html('');
			}
		}else{
			$('.charge .tip i').attr('class','');
			$('.charge .tip em').html('');
		}
