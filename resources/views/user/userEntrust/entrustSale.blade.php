                ...
                <dl>
                    <dd class="width1">
                        <p class="finish_data color8d">
                            <span class="color2d" style="float: left;margin-right: 10px;">@if($val->houseType1 == 3)[二手房]@elseif($val->houseType1 == 2)[楼售]@elseif($val->houseType1 == 1)[铺售]@endif</span>
                    ...
                    <dd class="dd2">
                        <span class="colorfe">{{floor($val->price2)}}</span>&nbsp;万元
                    </dd>
                    <dd class="comment_r">
                        @if($val->hadCommissioned <= 0)
                        <p>
                            <a @if($val->houseType1 == 3) href="/houseHelp/sale/xq/entrust?id={{$val->id}}" @elseif($val->houseType1 == 2) href="/houseHelp/sale/office/entrust?id={{$val->id}}" @elseif($val->houseType1 == 1) href="/houseHelp/sale/shop/entrust?id={{$val->id}}" @endif>
                                编辑
                            </a>
                        </p>
                        @endif
                        @if($val->entrustState == 1)
                            @if($val->hadCommissioned > 0)
                                <p><a href="/userEntrust/sale-{{$val->id}}">查看委托</a></p>
                            @endif
                            @if($val->hadCommissioned == 0)
                                <p><a onclick="delEntrust('{{$val->id}}','sale')">取消委托</a></p>
                            @endif
                        @endif
                        @if($val->entrustState == 0)
                            <p><a onclick="openAll('{{$val->id}}','sale')">开启委托</a></p>
                        @endif
                        {{--<p><a>转为直租</a></p>--}}
                    </dd>
                </dl>
            ...
            @else
                <div class="user_r">
                    <div class="no_data">
                        ...
                        <div class="no_info">
                            <p class="p1"><span>亲</span>，您暂无委托任何出售房源！</p>
                            <p>去<a href="/houseHelp/sale/xq/entrust">委托房源</a></p>
                        </div>
                    </div>
                </div>
            @endif
