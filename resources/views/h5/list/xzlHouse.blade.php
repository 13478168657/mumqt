...
<html>
	...
	<body >
		...
		<header class="house_header">
			...
			<h3 class="house_Htit fl storeHouse">写字楼@if($type == 'xzlrent')出租@else出售@endif</h3>
				<ul class="houseUl">
					<li><a href="/xzlrent/area">出租</a></li>
					<li><a href="/xzlsale/area">出售</a></li>
				</ul>
			<!--<a class="house_Hmag fl" href="##"></a>-->
			<a class="house_Hmap fr" href="/map/rent/office"></a>
		</header>
		<!--========================写字楼header开始===============================================-->
		...
		<nav class="house_Nnav">
			<ul class="house_Nlist" id="pagenavi">
				...
				<li class="Nlist_left fl"><span>@if(!empty($averageprice)){{$averageprices[$averageprice]}}@else @if($type == 'xzlsale')总价@else租金@endif @endif</span><i></i><b></b></li>
				...
				$.get(linkurl+str,{'page':++proSort.pageNum},function(data){
					...
					if(oData.length>=1){//判断是否有数据
						...
						for(var i=0;i<oData.length;i++){
							...
							ulli +="<ul>";
							ulli +="<li class='first_active'>"+oData[i]._source.officeLevel+"<i>|</i></li><li>"+oData[i]._source.houseType2+"<i>|</i></li><li>楼层:"+oData[i]._source.currentFloor+"/"+oData[i]._source.totalFloor+"层</li>";
							ulli += "</ul>";
							//房源标签
							var y = 0;
