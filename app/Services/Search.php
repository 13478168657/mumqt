...
class Search{
	...
	/**
	 * @param ListInputView $l 检索参数
	 * @param bool $isHouseFee 是否是收费用户发布的房源：false.不是 true.是
	 * @return mixed
	 */
	public function searchHouse(ListInputView $l, $isHouseFee = FALSE){
		$fields = "";
		$esQuerys = [];
		$esFilters = [];
		...
		if(!empty($l->must_not)){
			array_push($esQuerys, new ESQuery("must_not", $l->must_not));
		}
        // 已上架房源
        if(!empty($l->state)) {
            array_push($esQuerys, new ESQuery("state", $l->state));
        }


		/*csr项目查询特殊处理，总价10W以内的出售房源不显示 start*/
		if($this->searchType == 'hs'){
			$range = ['price2' => ['gte' => 10]];
		}else{
			$range = [];
		}
		$range = array_merge($range, $l->range);
		/*csr项目查询特殊处理，总价10W以内的出售房源不显示 end*/
		if(!empty($range)){
			array_push($esQuerys, new ESQuery("range", $range));
		}


		$json = $this->esIndex->SearchHouse($this->searchType, $fields, $esQuerys, $esFilters, $l->page, $l->pageset, $l->order, $l->asc, $isHouseFee);
	...
	public function searchCommunity(ListInputView $l){
		...
		$json = $this->esIndex->SearchCommunity($l->fields, $esQuerys, $esFilters, $l->page, $l->pageset, $l->order, $l->asc, $l->isNew);
		$result = json_decode($json);
		return $result;
	}
	...
	public function searchBroker(BrokerInputView $l){
		...
		if(!empty($l->idcardState)){
			array_push($esQuerys, new ESQuery("idcardState", $l->idcardState));
		}
		if(!empty($l->must_not)){
			array_push($esQuerys, new ESQuery("must_not", $l->must_not));
		}
		if(!empty($l->range)){
			array_push($esQuerys, new ESQuery("range", $l->range));
		}
		if(!empty($l->orderby)){
			$json = $this->esIndex->SearchBroker($fields, $esQuerys, $esFilters, $l->page, $l->pagesize, $l->orderby, $l->asc);
		}else{
	...
	public function searchBrokerById($id){
		$json = $this->esIndex->SearchBrokerByCid($id);
		$result = json_decode($json);
		return $result;
	}
	...
	public function searchHouseListByIds($arr){
		$ids['ids'] = $arr;
		$json = $this->esIndex->SearchHouseListByIds($ids, $this->searchType);
		$result = json_decode($json);
		return $result;
	}
	...
	public function getCommunityByCid($cid, $isNew = FALSE){
		$json = $this->esIndex->SearchCommunityByCid($cid, $isNew);
		$result = json_decode($json);
		return $result;
	}
	 ...
	 * @return type
	 */
	public function searchHouse4Group(ListInputView $l, $groupType, $isNewComm = TRUE){
		$esQuerys = [];
		$esFilters = [];
		if(!empty($l->keyword)){
			array_push($esQuerys, new ESQuery("keywords", $l->keyword));
		...
		$json = $this->esIndex->SearchCount($this->searchType, $esQuerys, $esFilters, $groupType, $l->asc, $isNewComm);
		$result = json_decode($json);
		return $result;
	}

	public function houseIndexUpdate(IndexUpdateEntity $iue){
		return $this->esIndex->UpdateHouse($this->searchType, $iue->getIndexId(), $iue->getFields(), $iue->getValue());
	}
	 ...
	 * @return string
	 */
	public function houseIndexDelete($indexId, $isHouseFee = FALSE){
		return $this->esIndex->DeleteHouse($this->searchType, $indexId, $isHouseFee);
	}
	 ...
	 * @return mixed
	 */
	public function brokerIndexDelete($indexId){
		return $this->esIndex->DeleteBrokersIndex($indexId);
	}

	public function communityIndexDelete($indexId, $isNew = FALSE){
		return $this->esIndex->DeleteCommunity($indexId, $isNew);
	}

	/**
	 * 批量删除房源索引(根据房源id数组)
	 * @param $indexId 房源id数组
	 * @return array
	 */
	public function houseDeleteByIdArrays($indexId){
		return $this->esIndex->DeleteHouseByIdsArray($this->searchType, $indexId);
	}
	...
	private function getCommunityPoints($searchResult){
		...
		foreach($searchResult->aggregations->group->buckets as $r){
			if($r->key > 0){
				...
				if($c != NULL){
					$p = new MapPoint($c->id, $c->name, $c->longitude, $c->latitude, 0, '', $c->subwayStationId);
					$p->count = $r->doc_count;
					array_push($points, $p);
				}
			}
	...
	private function getCityAreaPoints($cityId, $searchResult){
		...
		if(Cache::has($cacheKey)){
			...
			foreach($cityareas as $c){
				$p = new MapPoint($c->id, $c->name, $c->longitude, $c->latitude, 0, '', NULL);
...
//		}
	...
	public function ESAutoComplete($query_string, $type1, $isNew = FALSE){
		return $this->esIndex->ESAutoComplete($query_string, $type1, $isNew);
	}
}
